...
// Wrapper around KeyValueEditor for URL query params
...
export function ParamsEditor({ params, onChange }: ParamsEditorProps) {
  ...
  // Initialize data from params prop
  useEffect(() => {
    const newData = Object.entries(params).map(([key, value]) => ({
      key,
      value,
      enabled: true
    }));
  ...
  const handleChange = (newData: Array<{ key: string; value: string; enabled: boolean }>) => {
    ...
    // Convert array back to object, only including enabled params for the actual request
